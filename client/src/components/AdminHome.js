...
export default function AdminHome () {
    ...
    const notifyMessage = () => toast.success('🦄 Successfull', {
        ...
        useEffect(() => {
            try {
                ...
                Axios.get(`https://nlsuu-production.up.railway.app/admin-home/${filt}`, {
                    headers: AuthHeader()
                    })
                .then(response => {
    ...
    const deleteConfess = (optiontodelete, type) => {
        Axios.delete(`https://nlsuu-production.up.railway.app/confess/${optiontodelete}`, { headers: AuthHeader() })
    ...
    const updateConfess = (optiontoupdate, optiontodelete, type) => {
        Axios.post(`https://nlsuu-production.up.railway.app/admin-review`, {confess: optiontoupdate, group: type}, { headers: AuthHeader() })
        .then(res => {if(res.status === 200) 
            notifyMessage()
        }).then (Axios.delete(`https://nlsuu-production.up.railway.app/confess/${optiontodelete}`, { headers: AuthHeader() }))
